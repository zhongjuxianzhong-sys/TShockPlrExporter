...
- 汇总消息统一显示相对路径 `tshock/PlayerExports`，控制台不再回显又长又与面板实例目录绑死的绝对路径。
  绝对路径仍然写入 TShock 日志，需要时可查。

### 1.1.1

- 控制台执行时，结果消息不再经过主线程队列，改为直接写控制台与日志。此前「导出完成」这条消息
  依赖 `GameUpdate` 钩子投递，钩子一旦没有运行，命令就只有「已开始」而永远没有结果。
- 结果汇总无条件写入 TShock 日志，消息投递环节出问题时仍有可查的记录。
- 消息发送不再使用 `Color` 重载，统一走 TShock 的 `SendSuccessMessage` 等字符串接口。
- 后台任务不会再无声失败：`catch` 块里的日志调用套了不会抛异常的包装，任务本身的异常也会被观测并记录。
- 主线程队列记录最后一次抽取时间；长时间没被抽取时会在控制台报警，超时异常也会带上停摆时长。
- 数据库命令设置 30 秒超时，避免数据库无响应时导出任务永久挂住并一直占着单实例标志。
- 「已有导出任务正在执行」的提示带上已运行时长。
- 启动信息带上插件版本号，方便确认服务器实际加载的是哪一版。

### 1.1.0

- 正常路径不再改写 `Main.ServerSideCharacter`；回退方案限定在主线程执行。
- 导出移到后台线程，命令不再阻塞服务器；同一时间只允许一个导出任务。
- 新增 MySQL 存储后端支持。
- 导出前同步在线玩家的 SSC 数据。
- 文件名改为 `{账号名}-{账号ID}.plr`，并加固文件名与输出路径的安全性。
- 备份时间戳精确到毫秒并处理冲突；每个账号最多保留 10 份备份。
- 收敛会被当作数组下标的字段，避免客户端读档崩溃。
